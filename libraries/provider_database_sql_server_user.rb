        ...
        def action_create
          begin
            unless exists?(:logins)
              db.execute("SET ANSI_WARNINGS ON;SET CONCAT_NULL_YIELDS_NULL ON;SET ANSI_PADDING ON; CREATE LOGIN [#{@new_resource.username}] WITH PASSWORD = '#{@new_resource.password}', CHECK_POLICY = OFF").do
        ...
        def action_alter_roles
          begin
            ...
            @new_resource.sql_roles.each do | sql_role, role_action |
              alter_statement = "SET ANSI_WARNINGS ON;SET CONCAT_NULL_YIELDS_NULL ON; ALTER ROLE [#{sql_role}] #{role_action} MEMBER [#{@new_resource.username}]"
